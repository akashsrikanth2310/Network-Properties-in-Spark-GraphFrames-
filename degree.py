...
def simple(g):
	...
	# YOUR CODE HERE
    extractedFlipped = g.edges.select("dst","src")
	...
	# YOUR CODE HERE
    combinedEdges = extractedFlipped.unionAll(g.edges).distinct()
    filteredEdges = combinedEdges.filter("src!=dst")
    verticesNeeded = g.vertices
    return GraphFrame(verticesNeeded,filteredEdges)
    
''' Return a data frame of the degree distribution of each edge in the provided graphframe '''
def degreedist(g):
	# Generate a DF with degree,count
    # YOUR CODE HERE
    CountOfDegrees = g.inDegrees
    groupOfCountDegrees = CountOfDegrees.groupBy("inDegree").count() 
    return groupOfCountDegrees
...
def readFile(filename, large, sqlContext=sqlContext):
    ...
    # YOUR CODE HERE
    edgesSplit = lines.map(lambda line: line.split(delim))
    selectedVal =[]
    for line in edgesSplit.collect():
        value = Row(line[0],line[1])
        selectedVal.append(value)
    createdEdgesDataFrame = sqlContext.createDataFrame(selectedVal,['src', 'dst'])
    ...
    vertexSplit = lines.flatMap(lambda line: line.split(delim)).distinct()
    createdVerticesDataFrame = sqlContext.createDataFrame(vertexSplit.map(lambda x:(int(x),)),['id'])
    
    
	# Create graphframe g from the vertices and edges.
    g = GraphFrame(createdVerticesDataFrame,createdEdgesDataFrame)
    return g
